...
class OTUCalculator:
    ...
    def calculate_single_day(
        ...
        print(f"\n[1/4] Fetching NDVI (Median Composite ±{OTUConfig.gee.ndvi_window_days} days)...")
        ndvi_data = self._fetch_ndvi_for_chunks(target_date, use_cache, show_progress)
        
        # Step 2: Fetch soil data
        print("\n[2/4] Fetching soil raw data (SoilGrids)...")
        soil_data = self._fetch_soil_for_chunks(use_cache, show_progress)
        
        # Step 3: Fetch relief data
        print("\n[3/4] Fetching relief raw data (SRTM + Water)...")
        relief_data = self._fetch_relief_for_chunks(use_cache, show_progress)
        
        # Step 4: Calculate OTU for each chunk
        print("\n[4/4] Computing Q_OTU for each chunk...")
        ...
        print(f"  Q_OTU Mean: {stats['mean']:.3f}")
        print(f"  Q_OTU Std:  {stats['std']:.3f}")
        print(f"  Q_OTU Range: [{stats['min']:.3f}, {stats['max']:.3f}]")
    ...
    def _fetch_ndvi_for_chunks(
        ...
        try:
            ...
            for chunk in chunks_iter:
                if use_cache:
                    cached = self.chunk_manager.load_from_cache(chunk, "ndvi", target_date)
                    if cached is not None:
                        ndvi_data[chunk.id] = cached.get("value", 0.5)
                        continue
                
                try:
                    point = ee.Geometry.Point([chunk.center_lon, chunk.center_lat])
                    ...
                    ndvi_val = value.get("NDVI", 0.5) if value else 0.5
                    ndvi_val = max(0, min(1, ndvi_val)) if ndvi_val is not None else 0.5
                    ndvi_data[chunk.id] = ndvi_val
                    
                    if use_cache:
                        self.chunk_manager.save_to_cache(
                            chunk, "ndvi", target_date, {"value": ndvi_val}
                        )
                    
                except Exception as e:
                    print(f"  Warning: NDVI failed for chunk {chunk.id}: {e}")
                    ndvi_data[chunk.id] = 0.5
    ...
    def _fetch_soil_for_chunks(
        ...
        try:
            ...
            chunks_iter = self.chunk_manager.chunks
            if show_progress:
                chunks_iter = tqdm(chunks_iter, desc="Sampling soil", leave=False)
            
            for chunk in chunks_iter:
                if use_cache:
                    cached = self.chunk_manager.load_from_cache(chunk, "soil_raw", "static")
                    if cached is not None:
                        soil_data[chunk.id] = cached
                        continue
                ...
                try:
                    ...
                    if not val:
                        data = {"bd": 1300, "clay": 200, "soc": 50, "n": 2}
                    else:
                        data = {
                            "bd": val.get("bdod_0-5cm_mean", 1300),
                            "clay": val.get("clay_0-5cm_mean", 200),
                            "soc": val.get("soc_0-5cm_mean", 50),
                            "n": val.get("nitrogen_0-5cm_mean", 2),
                        }
                        # Handle None values
                    ...
                    soil_data[chunk.id] = data
                    
                    if use_cache:
                        self.chunk_manager.save_to_cache(chunk, "soil_raw", "static", data)
                    
                except Exception as e:
                    print(f"  Warning: Soil failed for chunk {chunk.id}: {e}")
    ...
    def _fetch_relief_for_chunks(
        ...
        try:
            ...
            chunks_iter = self.chunk_manager.chunks
            if show_progress:
                chunks_iter = tqdm(chunks_iter, desc="Sampling relief", leave=False)
            
            for chunk in chunks_iter:
                if use_cache:
                    cached = self.chunk_manager.load_from_cache(chunk, "relief_raw", "static")
                    if cached is not None:
                        relief_data[chunk.id] = cached
                        continue
                ...
                try:
                    ...
                    if not val:
                        data = {"slope": 0, "water": 0}
                    else:
                        data = {
                            "slope": val.get("slope", 0),
                            "water": val.get("occurrence", 0),
                        }
                        # Handle None
                        if data["slope"] is None: data["slope"] = 0
                        if data["water"] is None: data["water"] = 0
                    
                    relief_data[chunk.id] = data
                    
                    if use_cache:
                        self.chunk_manager.save_to_cache(chunk, "relief_raw", "static", data)
                    
                except Exception as e:
                    print(f"  Warning: Relief failed for chunk {chunk.id}: {e}")
